...
export class Chessboard2Component  {
    ...
    loadDefaultPgn() {
        if(localStorage.getItem("png")==undefined)
       { this.pgn = '1. c4 b5 2. cxb5 c6 3. bxc6 Nxc6 4. Qa4 a6\n' +
        ...
        this.setPgn();}
            ...
            this.pgn=localStorage.getItem("png")
            this.setPgn();
        }
    }
    passedData
 ...
 popup=false
  receiveMessage = (event: any) => {
    ...
    this.boardManager.move(message.data.move);
    if(message.data=="reset"){
        this.reset()
    }
    if(message.data.checkmate==true){
        this.popup=true
    }
    if(message.data.checkmate==false){
    localStorage.setItem("png", message.data.pgn.pgn);
    console.log(localStorage.getItem("png"))
